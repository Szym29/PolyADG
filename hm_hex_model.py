...
MS1_POS_PATH = 'data/mouse/sp_mouse/positive/'
MS1_NEG_PATH = 'data/mouse/sp_mouse/negative/'
MS2_POS_PATH = 'data/mouse/bl_mouse/positive/'
MS2_NEG_PATH = 'data/mouse/bl_mouse/negative/'
BATCH_SIZE = 64
PATCH_SIZE = 10
DEPTH = 16
NUM_HIDDEN = 64
...
def train(dataset, test_1,test_2,hyper_dict):
    ...
    with graph.as_default():
        ...
        tf_test_1_dataset = tf.constant(
            np.concatenate([test_1['train_dataset'], test_1['valid_dataset'], test_1['test_dataset']]))
        tf_test_1_label = tf.constant(
            np.concatenate([test_1['train_labels'], test_1['valid_labels'], test_1['test_labels']]))
        ...
        for motif in ms.MOUSE_MOTIF_VARIANTS:
            tf_motif_test_1_dataset[motif] = tf.constant(
                 test_1['motif_dataset'][motif]['test_dataset'])
            tf_motif_test_1_label[motif] = tf.constant(
                 test_1['motif_dataset'][motif]['test_labels'])

        tf_test_2_dataset = tf.constant(
            np.concatenate([test_2['train_dataset'], test_2['valid_dataset'], test_2['test_dataset']]))
        print(tf_test_2_dataset.shape)
        tf_test_2_label = tf.constant(
            np.concatenate([test_2['train_labels'], test_2['valid_labels'], test_2['test_labels']]))
        tf_motif_test_2_dataset = {}
        tf_motif_test_2_label = {}
        for motif in ms.MOUSE_MOTIF_VARIANTS:
            tf_motif_test_2_dataset[motif] = tf.constant(
                 test_2['motif_dataset'][motif]['test_dataset'])
            tf_motif_test_2_label[motif] = tf.constant(
        ...
        mlp_1_weights = tf.Variable(tf.truncated_normal(
            [896,2],stddev = tf_mlp_init_weight))
        ...
        concat_weights = tf.Variable(tf.truncated_normal(
            [2 + NUM_HIDDEN, NUM_LABELS],stddev = 1e-1))
        mlp_1_biases = tf.Variable(tf.constant(0.0, shape=[2]))
    ...
    # Predictions for the training, validation, and test data.
        ...
        _, test1 = model(tf_test_1_dataset, tf_test_1_label, drop=True)
        test1_prediction = tf.nn.softmax(test1)
        motif_test_1_prediction = {}
        for motif in ms.MOUSE_MOTIF_VARIANTS:


            _, motif_test_1 = model(tf_motif_test_1_dataset[motif], tf_motif_test_1_label[motif], drop=True)
            motif_test_1_prediction[motif] = tf.nn.softmax(motif_test_1)

        _, test2 = model(tf_test_2_dataset, tf_test_2_label, drop=True)
        test2_prediction = tf.nn.softmax(test2)
        motif_test_2_prediction = {}
        for motif in ms.MOUSE_MOTIF_VARIANTS:
            _, motif_test_2 = model(tf_motif_test_2_dataset[motif], tf_motif_test_2_label[motif], drop=True)
...
# Kick off training
    ...
    test_1_results = []
    test_2_results = []
    motif_train_results = {motif: [] for motif in hm.HUMAN_MOTIF_VARIANTS}
    motif_valid_results = {motif: [] for motif in hm.HUMAN_MOTIF_VARIANTS}
    motif_test_1_results = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    motif_test_2_results = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    save_weights = []
    with tf.Session(graph=graph) as session:
        ...
        np.random.seed()
        print('Initialized')
        print('Training accuracy at the beginning: %.1f%%' % accuracy(train_prediction.eval(), np.concatenate([dataset['train_labels'], dataset['test_labels']])))
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for step in range(shuffled_labels.shape[0] // BATCH_SIZE):
                ...
                session.run(stepOp)
            print('train loss', l)
            train_resuts.append(accuracy(train_prediction.eval(), np.concatenate([dataset['train_labels'], dataset['test_labels']])))
            ...
            for motif in ms.MOUSE_MOTIF_VARIANTS:
                ...
                motif_test_1_results[motif].append(
                    accuracy(motif_test_1_pred, test_1['motif_dataset'][motif]['test_labels']))
            ...
            for motif in ms.MOUSE_MOTIF_VARIANTS:
                ...
                motif_test_2_results[motif].append(
                    accuracy(motif_test_2_pred, test_2['motif_dataset'][motif]['test_labels']))
        ...
        # Early stopping based on validation results
            if epoch > 10 and valid_results[-11] > max(valid_results[-10:]):
                ...
                motif_test_2_results = {motif: motif_test_2_results[motif][:-10] for motif in ms.MOUSE_MOTIF_VARIANTS}
                return train_resuts, valid_results, test_1_results, motif_test_1_results, test_2_results, motif_test_2_results, \
                    save_weights[0], valid_losses,motif_train_results,motif_valid_results
    ...
    return train_resuts, valid_results, test_1_results, motif_test_1_results, test_2_results, motif_test_2_results, \
       save_weights[-1], valid_losses,motif_train_results,motif_valid_results


def main(_):
    # block_print()
    hyper_dict = gen_hyper_dict(HYPER_DICT)
    pos_data, pos_labels, neg_data, neg_labels = hm.produce_motif_dataset(NUM_FOLDS, HM_POS_PATH, HM_NEG_PATH)
    ms_1_pos_data, ms_1_pos_labels, ms_1_neg_data, ms_1_neg_labels = ms.produce_motif_dataset(NUM_FOLDS,
                                                                                                   MS1_POS_PATH,
                                                                                                   MS1_NEG_PATH)
    ms_2_pos_data, ms_2_pos_labels, ms_2_neg_data, ms_2_neg_labels = ms.produce_motif_dataset(NUM_FOLDS,
                                                                                                   MS2_POS_PATH,
                                                                                                   MS2_NEG_PATH)

    test_1_acc_split = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    test_2_acc_split = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    motif_train_accuracy_split = {motif: [] for motif in hm.HUMAN_MOTIF_VARIANTS}
    motif_valid_accuracy_split = {motif: [] for motif in hm.HUMAN_MOTIF_VARIANTS}
    # Cross validate
    train_accuracy_split = []
    valid_accuracy_split = []
    test_1_accuracy_split = []
    test_2_accuracy_split = []
    motif_test_1_accuracy_split = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    motif_test_2_accuracy_split = {motif: [] for motif in ms.MOUSE_MOTIF_VARIANTS}
    ...
    for i in range(NUM_FOLDS):
        ...
        save = hm.motif_data_split(pos_data, pos_labels, neg_data, neg_labels, NUM_FOLDS, split)
        save_2 = ms.motif_data_split(ms_2_pos_data, ms_2_pos_labels, ms_2_neg_data, ms_2_neg_labels, NUM_FOLDS,
                                          split)
        save_1 = ms.motif_data_split(ms_1_pos_data, ms_1_pos_labels, ms_1_neg_data, ms_1_neg_labels, NUM_FOLDS,
                                          split)
        train_split = {}
        valid_split = {}
        test_1 = {}
        ...
        for motif in ms.MOUSE_MOTIF_VARIANTS:
            ...
            test_1['motif_dataset'][motif]['test_dataset'], test_1['motif_dataset'][motif]['test_labels'] = pad_dataset(
                save_1['motif_dataset'][motif]['test_dataset'], save_1['motif_dataset'][motif]['test_labels'])
            ...
            test_1['motif_dataset'][motif]['test_dataset'] = np.concatenate([test_1['motif_dataset'][motif]['train_dataset'],test_1['motif_dataset'][motif]['valid_dataset'],
                                                                         test_1['motif_dataset'][motif]['test_dataset']])
            test_1['motif_dataset'][motif]['test_labels'] = np.concatenate([test_1['motif_dataset'][motif]['train_labels'],test_1['motif_dataset'][motif]['valid_labels'],
                                                                        test_1['motif_dataset'][motif]['test_labels']])
        ...
        for motif in ms.MOUSE_MOTIF_VARIANTS:
            ...
            test_2['motif_dataset'][motif]['test_dataset'], test_2['motif_dataset'][motif]['test_labels'] = pad_dataset(
                save_2['motif_dataset'][motif]['test_dataset'], save_2['motif_dataset'][motif]['test_labels'])
        ...
        train_resuts, valid_results, test_1_results, motif_test_1_results, test_2_results, motif_test_2_results, save_weights, valid_loss, motif_train_results,motif_valid_results = train(
            dataset, test_1, test_2, hyper_dict)
        print(valid_loss)
